...
const mobileMenuLinks: { href: string; label: string; icon: JSX.Element }[] = [
  ...
  // Services rendered separately as expandable
      ...
      <div className={`fixed inset-0 z-[99] flex flex-col px-6 overflow-y-auto pb-8 transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "translate-x-full"}`} style={{ background: "rgba(11,17,32,0.98)", backdropFilter: "blur(20px)", paddingTop: "calc(env(safe-area-inset-top, 0px) + 72px)" }}>
        {mobileMenuLinks.map((link, idx) => (
          <div key={link.href + link.label}>
            ...
            {/* Insert expandable Services right after Framework (idx === 1) */}
            {idx === 1 && (
              <div>
                <div className={`flex items-center gap-2 rounded-lg ${isActive("/services") || isActive("/cohort") || isActive("/ai-governance") || isActive("/advisory") ? "text-gold bg-white/5" : "text-muted-foreground"}`}>
                  <Link
                    to="/services"
                    onClick={() => setMobileOpen(false)}
                    className="flex-1 flex items-center gap-4 px-4 py-3.5 font-display text-base font-semibold tracking-[0.06em] hover:text-white"
                  >
                  ...
                  <button
                    ...
                    className="px-4 py-3.5 hover:text-white"
                  >
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`transition-transform duration-200 ${mobileServicesOpen ? "rotate-180" : ""}`}><polyline points="6 9 12 15 18 9"/></svg>
                  </button>
                </div>
                {mobileServicesOpen && (
                  <div className="ml-6 mt-1 mb-1 border-l border-white/10 pl-3 flex flex-col">
                    {servicesItems.map((item) => (
                      <Link
                        key={item.href + item.label}
                        to={item.href}
                        onClick={() => setMobileOpen(false)}
                        className={`block px-3 py-2.5 font-display text-[0.78rem] font-semibold tracking-[0.06em] rounded-md transition-all ${
                          item.highlight ? "text-gold hover:bg-gold/10" : "text-muted-foreground hover:text-white hover:bg-white/5"
                        }`}
                      >
                        {item.label}
                      </Link>
        ...
        <Link to="/services" onClick={() => setMobileOpen(false)} className="flex items-center justify-center mt-4 px-4 py-3.5 font-display font-semibold rounded-lg border border-gold/40 text-gold hover:bg-gold/10">
          Explore Services
        </Link>
        <Link to="/book" onClick={() => setMobileOpen(false)} className="flex items-center justify-center mt-2 px-4 py-3.5 font-display font-semibold rounded-lg" style={{ background: "#C85A1E", color: "#fff" }}>
