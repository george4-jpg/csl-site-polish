...
export default function ExecutiveGuideModal({ open, onClose }: ExecutiveGuideModalProps) {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        console.log("[ExecutiveGuide] Final request URL:", endpoint);
        console.log("[ExecutiveGuide] Payload being sent:", JSON.stringify(payload));
        ...
        try {
                ...
                console.log("[ExecutiveGuide] Response status:", res.status);
                console.log("[ExecutiveGuide] Raw response body:", text);
                if (!res.ok) {
                          throw new Error(`Status ${res.status}: ${text}`);
                }
                const data = JSON.parse(text);
                console.log("[ExecutiveGuide] Parsed response:", data);
          ...
          setGuideUrl(data?.guideUrl || GUIDE_FALLBACK_URL);
                ...
                console.error("[ExecutiveGuide] FAILED - caught error message:", err?.message, err);
